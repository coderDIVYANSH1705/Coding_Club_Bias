  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(panel3Ref.current,
        ...
        { opacity: 1, y: 0, scale: 1, duration: 2, ease: 'expo.out' }, 7.8);
  ...
  return (
    <>
      <style>{`
        ...
        .panel-abs { position:absolute; pointer-events:none; will-change:transform,opacity; padding-top:env(safe-area-inset-top); }
        @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0} }
        .cursor { animation:blink 1s steps(1) infinite; }
        @keyframes floatY { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }
        .float { animation:floatY 3.5s ease-in-out infinite; }
        .event-card:hover { border-color:rgba(0,255,136,0.35)!important; transform:translateY(-2px); }
        .btn-primary { padding:12px clamp(20px,4vw,30px); border-radius:8px; background:${G}; color:#050505; font-family:'JetBrains Mono',monospace; font-size:clamp(0.68rem,1.5vw,0.78rem); font-weight:700; letter-spacing:0.1em; border:none; cursor:pointer; box-shadow:0 0 22px rgba(0,255,136,0.4); transition:transform 0.2s ease,box-shadow 0.2s ease; pointer-events:auto; will-change:transform,box-shadow; }
        .btn-primary:hover { transform:translateY(-2px); box-shadow:0 0 36px rgba(0,255,136,0.6); }
        .btn-secondary { padding:11px clamp(20px,4vw,30px); border-radius:8px; background:transparent; color:rgba(255,255,255,0.65); font-family:'JetBrains Mono',monospace; font-size:clamp(0.68rem,1.5vw,0.78rem); font-weight:500; letter-spacing:0.1em; border:1px solid rgba(0,255,136,0.22); cursor:pointer; transition:all 0.2s ease; pointer-events:auto; will-change:border-color,color; }
        .btn-secondary:hover { border-color:rgba(0,255,136,0.55); color:${G}; }
        .p0-inner { display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:clamp(80px,14vw,120px) 20px clamp(20px,4vw,48px); width:100%; height:100%; box-sizing:border-box; }
        .stats-strip { display:flex; gap:clamp(14px,3.5vw,48px); justify-content:center; flex-wrap:wrap; padding:clamp(12px,2vw,18px) clamp(16px,3vw,28px); border-radius:14px; border:1px solid rgba(0,255,136,0.12); background:rgba(0,0,0,0.5); backdrop-filter:blur(14px); max-width:100%; }
        .p1-inner { display:flex; align-items:center; width:100%; height:100%; box-sizing:border-box; padding:clamp(80px,14vw,120px) clamp(16px,5vw,80px) clamp(24px,4vw,60px); }
        ...
        .p2-inner { display:flex; align-items:center; justify-content:flex-end; width:100%; height:100%; box-sizing:border-box; padding:clamp(80px,14vw,120px) clamp(16px,5vw,80px) clamp(24px,4vw,60px); }
        .p2-content { max-width:520px; width:100%; text-align:right; }
        .p3-inner { display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; width:100%; height:100%; box-sizing:border-box; padding:clamp(80px,14vw,120px) 20px clamp(48px,6vw,80px); }
        ...
        <div style={{ position:'sticky', top:0, width:'100%', height:'100vh', overflow:'hidden' }}>
          ...
          <div ref={panel0Ref} className="panel-abs" style={{ inset:0, zIndex:20 }}>
            <div className="p0-inner">
              ...
              <h1 style={{ ...headSt, fontSize:'clamp(2.8rem,12vw,9rem)', letterSpacing:'-0.02em', marginBottom:0, background:`linear-gradient(170deg,#ffffff 40%,${G} 100%)`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>
                CODING<br />CLUB
              </h1>
            ...
            <div className="p1-inner">
              <div className="p1-content">
                ...
                <h2 style={{ ...headSt, fontSize:'clamp(1.7rem,5vw,3.6rem)', marginBottom:10 }}>
                  What&apos;s<br />
                  <span style={{ background:`linear-gradient(90deg,${G},#fff)`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>Dropping Next.</span>
                </h2>
                <p style={{ ...bodySt, marginBottom:20, maxWidth:400 }}>Hackathons, competitive coding sprints, open-source jams — the calendar never sleeps.</p>
          ...
          <div ref={panel2Ref} className="panel-abs" style={{ inset:0, zIndex:20, opacity:0 }}>
            <div className="p2-inner">
              <div className="p2-content">
                ...
                <h2 style={{ ...headSt, fontSize:'clamp(1.7rem,5vw,3.6rem)', marginBottom:10 }}>
                  Learn.<br />
                  <span style={{ background:`linear-gradient(90deg,#fff,${G})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>Build. Ship.</span>
                </h2>
                <p style={{ ...bodySt, marginBottom:20, marginLeft:'auto', maxWidth:400 }}>Hands-on sessions with industry engineers. Docker, Kubernetes, Full-Stack, System Design — no fluff.</p>
          ...
          <div ref={panel3Ref} className="panel-abs" style={{ inset:0, zIndex:20, opacity:0 }}>
            <div className="p3-inner">
              ...
              <h2 style={{ ...headSt, fontSize:'clamp(2.2rem,8vw,6rem)', marginBottom:14, background:`linear-gradient(135deg,#ffffff 0%,${G} 100%)`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text', letterSpacing:'-0.02em' }}>
                INNOVATE.<br />DEPLOY.<br />SCALE.
              </h2>
              <p style={{ ...bodySt, maxWidth:'min(400px,85vw)', marginBottom:28 }}>Push your code to the limits. The terminal is yours — join the revolution.</p>
              <div style={{ display:'flex', gap:12, flexWrap:'wrap', justifyContent:'center' }}>
                <button className="btn-primary">&gt;_ JOIN THE CLUB</button>
